                          ...
                          PaTime output_latency, const char *format, int is_noninterleaved, double rate)
{
    ...
    if (!strcmp(format, "f32"))
        sample_format = paFloat32;
    else if (!strcmp(format, "i32"))
        sample_format = paInt32;
    else if (!strcmp(format, "i24"))
        sample_format = paInt24;
    else if (!strcmp(format, "i16"))
    ...
    if (err == paFormatIsSupported)
        printf("Open this stream as input is: OK\n");
    else
        printf("Open this stream as input is: NOK\n");

    err = Pa_IsFormatSupported(NULL, &expect_output_param, rate);
    if (err == paFormatIsSupported)
        printf("Open this stream as output is: OK\n");
    else
        printf("Open this stream as output is: NOK\n");
